...
namespace Expander.Controls
{
    ...
    public class ExpanderControl : HeaderedItemsControl
    {
        ...
        private const string TouchAreaRectangle = "TouchAreaRectangle";
        ...
        private Rectangle _touchAreaRectangle;
        ...
        public static readonly DependencyProperty NonExpandableHeaderProperty = DependencyProperty.Register(
            "NonExpandableHeader", typeof(bool), typeof(ExpanderControl), new PropertyMetadata(default(bool), OnNonExpandableHeaderPropertyChanged));

        public bool NonExpandableHeader
        {
            get { return (bool)GetValue(NonExpandableHeaderProperty); }
        ...
        {
            var source = (ExpanderControl)obj;
            source.OnNonExpandableHeaderChanged(e.NewValue as bool? ?? false);
        }

        protected virtual void OnNonExpandableHeaderChanged(bool newValue)
        {
            if (IsExpanded && newValue && (_header != null))
        ...
        private void OnPresenterSizeChanged(object sender, SizeChangedEventArgs e)
        {
            ...
            else if (null != _itemsCanvas && !IsExpanded)
            {
        ...
        internal virtual void UpdateVisualState(bool useTransitions)
        {
            ...
            if (_presenter != null)
            {
                ...
                {
                    _expandedToCollapsedFrame.Value = 0; //_presenter.DesiredSize.Height;
                }
            }
            if (IsExpanded)
            {
                if (NonExpandableHeader)
                    _header.Visibility = Visibility.Collapsed;
            ...
            var isNonExpandableState = IsNonExpandable ? NonExpandableState : ExpandableState;
        ...
        private void OnTouchAreaRectangleTap(object sender, TappedRoutedEventArgs e)
        {
            if (!IsNonExpandable)
            {
